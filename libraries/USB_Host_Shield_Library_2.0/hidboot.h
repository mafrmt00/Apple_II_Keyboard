...
#define UHS_HID_KEY_RIGHT 				0x4f // Keyboard Right Arrow
#define UHS_HID_KEY_LEFT 				0x50 // Keyboard Left Arrow
#define UHS_HID_BOOT_KEY_TAB			0x2b // Keyboard Tab
#define UHS_HID_BOOT_KEY_DELETE			0x4c // Keyboard Delete Forward
#define UHS_HID_BOOT_KEY_BACKSPACE		0x2a // Keyboard DELETE (Backspace)
#define UHS_HID_BOOT_KEY_ESC			0x29 // Keyboard ESCAPE
...
{
        static const uint8_t US_numKeys[10];
        static const uint8_t US_numKeysAlt[10];
        static const uint8_t US_symKeysUp[12];
        static const uint8_t US_symKeysLo[12];
        static const uint8_t US_padKeys[5];
        
        static const uint8_t DE_numKeys[10];
        static const uint8_t DE_numKeysAlt[10];
        static const uint8_t DE_symKeysUp[12];
        static const uint8_t DE_symKeysLo[12];
        static const uint8_t DE_padKeys[5];        
...
public:
		SetLayout(uint8_t uiLayout)
		{
			uiCurrectSelectedLayout = uiLayout;
			
			if (uiCurrectSelectedLayout > 1)
				uiCurrectSelectedLayout = 1;
		}
...
protected:
        ...
        virtual const uint8_t *getNumKeysAlt() 
        {
        	if( 0 == uiCurrectSelectedLayout)
        	{
                return US_numKeysAlt;
            }
            else
            {
            	return DE_numKeysAlt;
            }
        };
